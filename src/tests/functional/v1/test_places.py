...
class TestPlaces:
    ...
    async def test_create_place(self, client, session, httpx_mock):
        ...
        assert place["country"] == mock_response["countryCode"]
        assert place["city"] == mock_response["city"]
        assert place["locality"] == mock_response["locality"]
    ...
    async def test_auto_route(self, client, session, httpx_mock):
        ...
        coordinates = {"latitude": 12.34, "longitude": 23.45}
    ...
    async def test_get_list(self, client, session):
        ...
        await PlacesRepository(session).create_model(mock_body)
        ...
        self.assert_result(place, mock_response)
    ...
    async def test_get_one(self, client, session):
        ...
        # создание записи в БД
        created_id = await PlacesRepository(session).create_model(mock_body)
        # осуществление запроса
